...
Split: first 1000 samples -> train, last 200 -> test (the ntrain=1000/ntest=200
convention used throughout Zongyi Li's own Geo-FNO loading scripts for this file).
"""
import argparse
import os
...
def main():
    ...
    ap.add_argument("--out-dir", default="dataset/benchmarks/elasticity")
    args = ap.parse_args()
    ...
    write_split(os.path.join(args.out_dir, "elasticity_train.h5"), xy, sigma, list(range(0, 1000)))
    write_split(os.path.join(args.out_dir, "elasticity_test.h5"), xy, sigma, list(range(1800, 2000)))
