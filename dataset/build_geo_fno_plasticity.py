...
Split: first 900 samples -> plasticity.h5 (train, further split internally by
the loader), last 87 -> plasticity_seed42_test.h5 (held-out test) -- following
this repo's existing config naming, exact counts are a reasonable round split,
not a bit-exact paper replication.
"""
import argparse
import os
...
def main():
    ...
    ap.add_argument("--out-dir", default="dataset/benchmarks/plasticity")
    args = ap.parse_args()
    ...
    write_split(os.path.join(args.out_dir, "plasticity.h5"), inp, out, list(range(0, 900)), edges)
    write_split(os.path.join(args.out_dir, "plasticity_seed42_test.h5"), inp, out, list(range(900, 987)), edges)
