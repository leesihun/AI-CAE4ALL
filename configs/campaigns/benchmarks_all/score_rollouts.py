#!/usr/bin/env python3
"""Score configs/campaigns/benchmarks_all rollouts against their exN_infer.h5 ground
truth: relative L2 and R2 (coefficient of determination), per channel and
aggregate.

Reads configs/campaigns/benchmarks_all/roster.tsv for the (label, canonical train
...
scoring starts at t=1 like configs/campaigns/ex2/score_rollouts.py already does.

Usage:
    python configs/campaigns/benchmarks_all/score_rollouts.py
    python configs/campaigns/benchmarks_all/score_rollouts.py --labels meshgraphnets_ex8 transolver_ex8
    python configs/campaigns/benchmarks_all/score_rollouts.py --csv output/benchmarks_all/scores.csv
"""
...
def resolve(method_dir, value):
    """Resolve a native config value the way the launcher does.

    Native processes run with their working directory set to the method
    repository, so every path in a config is relative to methods/<name>/.
    """
    base = os.path.join(REPO_ROOT, 'methods', method_dir)
...
def main():
    ...
    if not all_rows:
        print("\nNo rollouts scored. Run configs/campaigns/benchmarks_all/train_all.sh then infer_all.sh first.")
        return
