#!/usr/bin/env bash
# Head-to-head TRAIN runner for the ex3 dataset (NASA-CRM, dataset/ex3_NASA_CRM_{coarse,orig}.h5).
#
# Trains all seven baselines against ex3 IN PARALLEL, each pinned to its own GPU
# (see gpu_for() below -- needs 7 free GPUs, e.g. an 8-GPU box), so the resulting
# checkpoints are directly comparable via configs/ex3/infer_all.sh. Each method's
...
#
# ex3 has TWO mesh resolutions of the same 105-sample NASA-CRM case (steady
# external aero, num_timesteps=1): coarse (40,000 nodes) and orig (454,404
# nodes). Select which one to run with DATASET_VARIANT (default: coarse).
#
...
# hierarchical HI-MGN backbone. Transolver, and the four Neural_Operator
# backends (deeponet/fno/gino/point_deeponet), round out the seven baselines --
# same set as configs/ex1/train_all.sh and configs/ex2/train_all.sh.
#
...
#   LOG_ROOT = directory for transcript logs (default: output/ex3_head_to_head/<variant>/train_logs)
#
# Usage:
#   bash configs/ex3/train_all.sh
#   DATASET_VARIANT=orig bash configs/ex3/train_all.sh
#   METHODS="meshgraphnets meshgraphnets-hi" bash configs/ex3/train_all.sh
#   PARALLEL=0 bash configs/ex3/train_all.sh
#   watch progress:  tail -f output/ex3_head_to_head/coarse/train_logs/train_fno.log
...
LOG_ROOT="${LOG_ROOT:-output/ex3_head_to_head/$DATASET_VARIANT/train_logs}"
...
config_for() {
    case "$1" in
        meshgraphnets)     echo "configs/MeshGraphNets/ex3/config_train_meshgraphnets_${DATASET_VARIANT}.txt" ;;
        meshgraphnets-hi)  echo "configs/MeshGraphNets/ex3/config_train_himgn_${DATASET_VARIANT}.txt" ;;
        deeponet)          echo "configs/Neural_Operator/ex3/config_train_deeponet_${DATASET_VARIANT}.txt" ;;
        fno)               echo "configs/Neural_Operator/ex3/config_train_fno_${DATASET_VARIANT}.txt" ;;
        gino)              echo "configs/Neural_Operator/ex3/config_train_gino_${DATASET_VARIANT}.txt" ;;
        point_deeponet)    echo "configs/Neural_Operator/ex3/config_train_point_deeponet_${DATASET_VARIANT}.txt" ;;
        transolver)        echo "configs/Transolver/ex3/config_train1_${DATASET_VARIANT}.txt" ;;
        *) echo "" ;;
    esac
}
...
ended=$(date +%s)
echo ""
echo "ex3 train-all ($DATASET_VARIANT) finished in $((ended - started))s (rc=$rc)."
echo "Transcripts:     $LOG_ROOT/train_<method>.log"
echo "Runtime configs: $RUNTIME_CONFIG_ROOT/"
if [ "$rc" = "0" ]; then
    echo "Next: DATASET_VARIANT=$DATASET_VARIANT bash configs/ex3/infer_all.sh"
fi
exit $rc
