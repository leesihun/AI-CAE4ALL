#!/usr/bin/env bash
# Head-to-head TRAIN runner for the ex2 dataset (dataset/ex2.h5).
#
# Trains all seven baselines against ex2.h5 (50 samples x 50 timesteps,
# autoregressive rollout target) IN PARALLEL, each pinned to its own GPU (see
# gpu_for() below -- needs 7 free GPUs, e.g. an 8-GPU box), so the resulting
# checkpoints are directly comparable via configs/ex2/infer_all.sh. Each method's
...
#   LOG_ROOT = directory for transcript logs (default: output/ex2_head_to_head/train_logs)
#
# Usage:
#   bash configs/ex2/train_all.sh
#   METHODS="meshgraphnets meshgraphnets-hi" bash configs/ex2/train_all.sh
#   PARALLEL=0 bash configs/ex2/train_all.sh
...
LOG_ROOT="${LOG_ROOT:-output/ex2_head_to_head/train_logs}"
...
ended=$(date +%s)
echo ""
echo "ex2 train-all finished in $((ended - started))s (rc=$rc)."
