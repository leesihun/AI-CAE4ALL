#!/usr/bin/env bash
# dataset_matrix campaign, machine 135: every train+infer arm of this machine's
# half of configs/campaigns/dataset_matrix/manifest.json, on 8 GPUs.
#
#   bash configs/run_all_135.sh                 (run inside tmux, or under nohup)
#   DRY_RUN=1 bash configs/run_all_135.sh       print the plan, write nothing
...
exec "$PY" "$here/campaigns/dataset_matrix/run_matrix.py" --machine 135 "$@"
