...
Three failures have already cost us a run, and NONE of them raises:
  ...
  1. `eval_dataset` absent from the config. rollout.py prints
     "Skipped: no 'eval_dataset' set in config" and exits 0. The run reports
     a clean finish with no histogram -- the one output the campaign is for.
...
def main():
    ...
    if not configs:
        ...
        print("      The configs are checked in beside this script; "
              "--config-dir may be pointing elsewhere.", file=sys.stderr)
        return 1
    ...
    if bad:
        ...
        print("Fix before spending GPU time. Most likely, in order:",
              file=sys.stderr)
        print("  1. wrong pairing   eval_dataset must be the _compare_ file "
              "of the SAME part as infer_dataset", file=sys.stderr)
