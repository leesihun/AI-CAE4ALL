#!/usr/bin/env python3
"""Dynamic shared-queue campaign runner for configs/benchmarks_all/roster.tsv.
...
directly (e.g. on Windows without bash):
    ...
    python configs/benchmarks_all/campaign_runner.py --mode train
    CHECK_ONLY=1 python configs/benchmarks_all/campaign_runner.py --mode train --gpus 0
...
def main(argv: list[str]) -> int:
    ...
    if rc == 0:
        if args.mode == "train":
            print("Next: bash configs/benchmarks_all/infer_all.sh")
        else:
            print("Next: python configs/benchmarks_all/score_rollouts.py")
    return rc
