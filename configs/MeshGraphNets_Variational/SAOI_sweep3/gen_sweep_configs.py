...
def render_infer(src_lines, arm, tag, gpu, values):
    ...
    for line in src_lines:
        ...
        if key in ('dataset_dir', 'infer_dataset'):
            ...
            # source of truth for where the data lives, so a repo-layout change
            # (e.g. method repos moving under methods/, which changes the
            # relative depth) only has to be fixed there.
            #
            # The production infer configs historically spelled the directory
            # `SAOI` while every training config spells it `saoi`. Both keys are
            # PATH_KEYS, so the parser preserves case and exactly one spelling
