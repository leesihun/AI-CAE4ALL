...
# The long peak-to-valley run: BOT and TOP sections x beta_aux {0, 100},
# ONE GPU PER ARM. 8-way DDP measured 0.17x a single card, so the GPUs go to
# parallel arms instead of data parallelism. beta_aux 0 is the control: the old
...
if [ "$SCORE" = "1" ] && [ "$INFL" = "1" ]; then
    ...
    # rank_arms.py reads ONLY the spread dumps, so the arm naming is
    # irrelevant to it -- unlike score_sweep.py, which keys arms to positions
    # in the retired 2^(4-1) design table and would silently produce empty
    # main-effect tables for this roster.
    echo "================= RANKING ================="
    "$PYTHON" configs/campaigns/rank_arms.py "output/meshgraphnets-v/saoi_sweep3/infer" 2>&1 | tee "$LOG_ROOT/rank_arms.log"
    echo "=========================================="
    echo ""
    # The document: the roster, every metric, the ranking, and a COMPUTED
    # verdict on the question this run was launched to answer.
    if "$PYTHON" configs/campaigns/write_report.py \
            --kind mgnv --axis beta_aux \
            --label "MeshGraphNets-V peak-to-valley (beta_aux)" \
            --infer "output/meshgraphnets-v/saoi_sweep3/infer" \
            --logs "$SCORE_OUT" \
            --configs "$CFG_DIR" \
            --arms "$ARMS" \
            --out "docs/research/SAOI_LONG_RUN_MGNV.md" > "$LOG_ROOT/write_report.log" 2>&1; then
        cp "docs/research/SAOI_LONG_RUN_MGNV.md" "$REPORT" 2>/dev/null || true
        echo "Report written."
    else
    ...
    if [ -f "$REPORT" ]; then
        ...
        echo "==========================================="
        echo ""
        echo "Document : docs/research/SAOI_LONG_RUN_MGNV.md   <-- committed with the repo"
        echo "Copy     : $REPORT"
        echo "Ranking  : $LOG_ROOT/rank_arms.log"
    fi
else
    echo "SCORE=0 -- skipped. Run it later with:"
    echo "  $PYTHON configs/campaigns/rank_arms.py output/meshgraphnets-v/saoi_sweep3/infer"
