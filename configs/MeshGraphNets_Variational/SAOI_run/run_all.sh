#!/usr/bin/env bash
# MeshGraphNets-V SAOI: train, then infer.
#
#   nohup bash configs/MeshGraphNets_Variational/SAOI_run/run_all.sh \
#       > output/meshgraphnets-v/saoi_run/run.out 2>&1 &
#
...
# not variants of one recipe -- the recipe is fixed, and is the P0 cell of the
# retired FM-v2 ablation (see README.md). Each half gets its own GPU and the
# two halves run in parallel; the three evaluation sets of one half run in
# series on that half's card.
#
# Useful overrides:
...
CFG_DIR="$SCRIPT_DIR"
OUT_ROOT="output/meshgraphnets-v/saoi_run"
...
echo "=================================================================="
echo " MeshGraphNets-V SAOI: train -> infer -> histogram"
echo "=================================================================="
echo "  arms      : $ARMS"
