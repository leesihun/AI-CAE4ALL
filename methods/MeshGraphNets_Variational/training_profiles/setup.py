...
def build_optimizer_scheduler(config, params, total_epochs: int):
    """
    Build fused Adam with optional coupled L2 weight decay and a SequentialLR:
    linear warmup then cosine warm restarts.
...
def _artifact_dir(config) -> str:
    ...
    return (config or {}).get('log_dir') or '../../output/meshgraphnets-v'
