...
def test_conditions_reach_the_encoder_weights(datasets):
    ...
    config['num_timesteps'] = 1
    model = MeshGraphNets(config, 'cpu')
    out = model(train[0], add_noise=False)
    predicted, target = out[0], out[1]
...
def test_ar_rt_rollout_carries_conditions_unchanged(datasets):
    ...
    # the variational tree composes the full VAE objective per step, so its
    # rollout_loss takes compose_loss(graph) -> (prediction, loss, recon_sum, ...)
    def compose_loss(g):
        out = model(g, add_noise=False)
        predicted, target = out[0], out[1]
        per_element = torch.nn.functional.mse_loss(predicted, target, reduction='none')
        return predicted, per_element.mean(), per_element.sum()

    loss = rollout_loss(model, graph, ctx, compose_loss, training=False)[0]
