...
def _try_delete(path: str) -> bool:
    ...
    then silently fall back to rebuilding hierarchies per sample per epoch —
    so there we take a non-blocking exclusive ``flock`` first and skip the
    delete if any reader holds the shared lock (:class:`HierarchyCacheReader`).
