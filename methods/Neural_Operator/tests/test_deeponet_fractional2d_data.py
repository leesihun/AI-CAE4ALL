...
FRACTIONAL_BENCHMARK_DIR = (
    SUITE_ROOT / "dataset" / "benchmarks" / "deeponet_fractional2d"
)
FRACTIONAL_PREPARE = FRACTIONAL_BENCHMARK_DIR / "prepare_fractional2d.py"
FRACTIONAL_TRAINER = FRACTIONAL_BENCHMARK_DIR / "train_fractional2d.py"
requires_fractional_prepare = pytest.mark.skipif(
    not FRACTIONAL_PREPARE.is_file(),
    reason="optional DeepONet fractional-2D preparation helper is not present",
)
requires_fractional_trainer = pytest.mark.skipif(
    not FRACTIONAL_TRAINER.is_file(),
    reason="optional DeepONet fractional-2D training helper is not present",
)
