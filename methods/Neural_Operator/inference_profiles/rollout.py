...
def run_rollout(config, config_filename='config.txt'):
    ...
    output_dir = config.get('inference_output_dir', '../../output/neural_operator/rollout')
    ...
    for sample_id in sample_ids:
        ...
        with torch.no_grad():
            for step in range(steps):
                ...
                graph.batch = batch_t
                graph.ptr = ptr_t
                # Point-DeepONet seeds its deterministic sensor sampling
                # from the sample id.
...
def _write_rollout_output(output_dir, sample_id, ref_pos, mesh_edge, all_states,
    ...
    with h5py.File(tmp_path, 'w') as f:
        ...
        f.attrs['output_var'] = int(output_var)
