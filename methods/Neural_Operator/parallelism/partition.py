...
Ported unchanged from MeshGraphNets' parallelism/partition.py: given per-block
memory estimates and a target number of stages K, find an assignment of
consecutive blocks to stages such that the max stage cost is minimized.
Classic O(L^2 * K) dynamic program.
"""
...
def partition_stages(
    ...
    Args:
        ...
        entry_penalty: optional per-block cost added to a stage that *begins* at
            that block. Used to charge a stage for boundary tensors it must
            receive and hold across its lifetime, biasing cuts toward cheap
            boundaries.
