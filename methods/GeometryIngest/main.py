...
native entrypoint: ``python methods/GeometryIngest/main.py --config <file>``.
The launcher runs this with the working directory set to this repository
(``methods/GeometryIngest``), so relative config paths resolve from here.
...
# Put the parent (methods/) on sys.path so this script can import the
# GeometryIngest package regardless of the launcher's working directory.
sys.path.insert(0, str(Path(__file__).resolve().parent.parent))

from GeometryIngest.config import load_config, params_from_config  # noqa: E402
from GeometryIngest.pipeline import gather_paths, run_ingest        # noqa: E402
