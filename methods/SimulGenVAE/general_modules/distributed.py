...
def cuda_enabled():
    """CUDA usable for this run. The test harness sets SDFFLOW_FORCE_CPU_DIST=1
    to exercise the gloo/CPU path even on a box that has a GPU."""
    return torch.cuda.is_available() and os.environ.get('SDFFLOW_FORCE_CPU_DIST') != '1'
...
def init_process_group(rank, world_size, gpu_id, port):
    ...
    backend = 'nccl' if cuda_enabled() else 'gloo'
...
def wrap_model(model, config, device):
    ...
    if cuda_enabled() and device.type == 'cuda':
        wrapped = DDP(model, device_ids=[device.index], broadcast_buffers=True,
