...
def build_optimizer_scheduler(config, params, total_epochs: int):
    ...
    optimizer = torch.optim.Adam(
        params, lr=learning_rate, weight_decay=weight_decay, fused=use_fused
    )
...
def build_model_config(config) -> dict:
    ...
    return {
        ...
        'periodic_box': config.get('periodic_box'),
...
def save_checkpoint(
    ...
    # A GUI run commonly points at a fresh, run-specific output directory. The
    # log path is optional, so init_log_file() cannot be relied on to create the
    # checkpoint parent first.
    model_parent = os.path.dirname(os.path.abspath(modelpath))
...
def _artifact_dir(config) -> str:
    """Where a run's files belong: the log's directory, else the run root."""
    return (config or {}).get('log_dir') or '../../output/chi-mgnflow'
...
def init_log_file(config, config_filename: str):
    ...
    cwd-relative directory, so every artifact of a run lands together.
