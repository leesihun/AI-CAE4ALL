...
def construct_log_path(config):
    """The trainer writes to log_file_dir verbatim, relative to its own cwd."""
    return os.path.normpath(config.get('log_file_dir', 'train.log'))
...
def plot_loss(epochs, train_losses, valid_losses, config, output_path=None):
    ...
    if output_path is None:
        output_dir = os.path.dirname(os.path.normpath(
            config.get('log_file_dir', 'train.log'))) or '.'
...
def main():
    ...
    parser.add_argument('--output', type=str, default=None,
                       help='Output image path (default: next to the log file)')
