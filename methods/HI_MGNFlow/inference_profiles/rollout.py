...
def _load_model_from_checkpoint(config, checkpoint, device):
    """Rebuild the velocity network from a checkpoint and load (EMA-preferred) weights."""
...
def run_rollout(config, config_filename='config.txt'):
    ...
    # Keys the checkpoint records for PROVENANCE but must not impose at
    # inference. flow_steps / flow_solver are quadrature choices on a learned
    # continuous velocity field: the same weights integrate correctly at any K,
    # and letting the checkpoint pin them would silently ignore the config and
    # contradict the one property that makes this method cheap to tune.
    # flow_time_freqs is deliberately NOT in this set -- it sets the AdaLN input
    # width, so it is genuinely architecture-defining and the checkpoint wins.
    SAMPLING_TIME_KEYS = ('flow_steps', 'flow_solver')
    ...
    if 'model_config' in checkpoint:
        ...
        for k, v in model_config.items():
            ...
            if k in SAMPLING_TIME_KEYS:
                if old_val is not None and old_val != v:
                    print(f"    {k}: {old_val} (config wins; checkpoint recorded {v})")
                else:
                    config[k] = v
                    print(f"    {k}: {v}")
                continue
    ...
    if flow_cfg['predict'] == 'mean':
        print(f"  Flow readout: DETERMINISTIC mean -- 1 forward per scene, no "
              f"integration. num_vae_samples is ignored (every draw is identical).")
        num_vae_samples = 1
    ...
    output_dir = config.get('inference_output_dir', '../../output/chi-mgnflow/rollout')
