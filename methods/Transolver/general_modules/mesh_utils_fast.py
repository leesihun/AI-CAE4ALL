"""
Mesh visualization/writer utilities, adapted from Neural_Operator's
general_modules/mesh_utils_fast.py (itself adapted from MeshGraphNets).

Same divergence from the MGN source: this repository's runtime graph contract
has no `edge_attr` -- Transolver is an attention model and consumes no edge
features, it carries `edge_index` only so the surface can be reconstructed for
visualization -- so the edge-attribute dataset is dropped from the HDF5 writer.
Triangle reconstruction, face-value averaging, and PyVista rendering are
otherwise unchanged.

Used by training_profiles/training_loop.py::test_model under `display_testset`.
The separate `write_test_predictions` dump is a different, leaner file: numeric
arrays under <log_dir>/dumps/, while this writer's HDF5 + PNG pair lands under
<log_dir>/<prefix>/ (matching Neural_Operator's layout).
"""
...
def save_inference_results_fast(output_path, graph,
    ...
    Args:
        ...
        graph: PyG Data object with pos, edge_index, sample_id, time_idx
    ...
    if part_ids is not None and faces.shape[0] > 0:
        # Use the first vertex's part (they should all agree for valid meshes)
        face_part_ids = part_ids[faces[:, 0]]
    ...
    with h5py.File(output_path, 'w') as f:
        ...
        # Edge data (topology only; this repo has no edge_attr)
...
def plot_mesh_comparison(pos, faces, pred_values_norm, target_values_norm,
    ...
    r2_str_norm   = f'r2: {r2_norm:.4f}'
    r2_str_denorm = f'r2: {r2_denorm:.4f}'
