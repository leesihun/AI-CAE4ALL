...
def _as_list(value):
    """Normalize one-value config lines to the list contract used by membership checks."""
    if value is None:
        return []
    ...
    with torch.no_grad():
        ...
        for batch_idx, graph in enumerate(pbar):
            ...
            if delta_mean is not None and delta_std is not None:
                ...
                if write_hdf5 and batch_idx in _as_list(config.get('test_batch_idx', [0, 1, 2, 3])):
                    ...
                    dump_root = config.get('log_dir') or '../../output/transolver'
                    output_path = os.path.join(
                        dump_root, 'dumps', output_prefix, str(gpu_ids), str(epoch),
                        f'{filename}.h5')
