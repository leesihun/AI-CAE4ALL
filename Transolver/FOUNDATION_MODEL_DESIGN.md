...
- 토큰화: `_chunk_stats` ([physics_attention.py:139-154](model/physics_attention.py#L139-L154))
- 토큰 간 attention: `_slice_attend` ([physics_attention.py:156-164](model/physics_attention.py#L156-L164))
- 역투사: `_deslice` ([physics_attention.py:166-179](model/physics_attention.py#L166-L179))
...
2. **외삽에서 A와 B가 같이 무너진다 (69→94%, 56→97%).** 토큰은 외삽을 만들지 못한다.
   ...
   ([mesh_dataset.py:499](general_modules/mesh_dataset.py#L499),
   통계는 [:252](general_modules/mesh_dataset.py#L252)) head가 한 번도 낸 적 없는
   정규화 값을 내야 한다.
...
- 타깃: `target_norm = (target_delta - delta_mean) / delta_std`
  ([mesh_dataset.py:499](general_modules/mesh_dataset.py#L499))
- 좌표: 전역 RMS 반경 하나로 나눔
  ([mesh_dataset.py:22-32](general_modules/mesh_dataset.py#L22-L32))
- node type: **닫힌 vocabulary** — 처음 보는 part number면 하드 에러
  ([mesh_dataset.py:312-328](general_modules/mesh_dataset.py#L312-L328))
...
([physics_attention.py:102](model/physics_attention.py#L102),
[:110-113](model/physics_attention.py#L110-L113)). foundation model에서는 토큰
...
([physics_attention.py:130-137](model/physics_attention.py#L130-L137)) 안에서 `[H, N, 1]`
...
예리도**를 뜻한다 ([Transolver.py:30-32](model/Transolver.py#L30-L32)). 여기에 열 BC를
...
> [amortized.py:37-39](model/amortized.py#L37-L39)에 이미 기록된 교훈이 그대로 적용된다.
...
- `_slice_attend(tokens, cond)` → `[M ; K]` concat, attend, **앞 M개만** deslice로.
  ...
  [`_forward_naive`:203](model/physics_attention.py#L203),
  [`_forward_slice_space`:301](model/physics_attention.py#L301),
  [`decode_with_tokens`:341](model/physics_attention.py#L341).
...
- part 2가 ex1에서는 107 노드, ex2에서는 4096 노드인데 **one-hot 슬롯이 같다.** 합치면
  ...
  ([mesh_dataset.py:330-349](general_modules/mesh_dataset.py#L330-L349)).
...
> ([mesh_dataset.py:445](general_modules/mesh_dataset.py#L445)) part-id 집합도 전
...
([Transolver.py:57](model/Transolver.py#L57))를 버리고 **채널별 임베딩 + field token**으로
