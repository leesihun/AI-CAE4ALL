"""Normalization-statistics computation for MeshGraphDataset (Transolver).

Accumulates element-weighted first/second moments over all nodes and target
deltas of a sample split, optionally in parallel across processes, plus the
isotropic position scale used by centered_isotropic coordinate normalization
(IMPLEMENTATION_PLAN.md section 5.5). Baseline Physics-Attention does not
consume edges, so unlike MeshGraphNets' dataset_stats.py there are no edge
statistics here.
"""
...
def finalize_position_scale(sqnorm_sum: float, count: int) -> float:
    """RMS radius of per-sample-centered reference geometry (section 5.5)."""
    if count <= 0:
    ...
    with h5py.File(h5_file, 'r') as f:
        for sid in sample_ids:
            try:
                ...
                # Positional features and position_scale depend on reference
                # geometry only — computed once per sample from timestep 0.
...
def compute_normalization_stats(h5_file: str, sample_ids: List[int], input_dim: int,
    ...
    `finalize_moments` on the sum/sumsq/count triples for mean/std, and
