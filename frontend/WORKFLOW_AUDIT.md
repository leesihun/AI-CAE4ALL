...
| The browser suite covered core workflows but left many visible controls reachable only by inspection, not actual clicks. Uploaded paths containing a nested `dataset/` segment were also rewritten to the wrong repository root. | A button could remain unbound or silently drop values even while the main happy path passed; a locally uploaded HDF5 could not be reopened in the viewer. | Added an inventory-driven control-surface regression for editor, config, model, System, deploy, catalogs, viewer, evaluation, comparison, optimization, export, navigation, and runtime actions. Path normalization now selects the earliest exact repository-root segment, preserving `frontend/runtime/uploads/...`; model rows expose a real keyboard-accessible Details button. | Fifteen Chrome runners pass, including a real DeepJEB closed-loop native launch. The inventory finds 137 ID-bearing interactive controls; the only three not named directly are hidden file inputs, each exercised through its visible file-chooser button. |
| Live `defaults_by_mode` stopped at the API boundary, and the newly registered SDFFlow `optimize` keys had neither a dedicated information surface nor useful field guidance. | The launcher could validly default a required field while the GUI falsely called it missing; users also had to decode latent-space, meshing, material, constraint, CMA-ES, and verification keys from source comments. | Preserve global and per-mode backend defaults with explicit provenance, exclude them from missing-required diagnostics without silently serializing them, group `opt_*` controls under Optimization, add field help, and provide a confirmation-gated closed-loop preset. | Main Chrome regression compares every trainable route's modes/keys/requirements/defaults to the live MethodSpec, selects `optimize`, inspects an implicit default, and clicks the preset into an explicit config. |
