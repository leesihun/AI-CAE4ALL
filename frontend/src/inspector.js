...
import { duplicateNode, deleteSelected } from "./graph.js";
import { openConfig } from "./config.js";
import { openArtifact } from "./viewer.js";
import { runGraph } from "./run.js";
import { openStudio, renderStudio, liveShell, liveError } from "./studio.js";
...
export function renderInspector() {
  ...
  const configEntries = Object.entries(node.config).slice(0, spec.isModel ? 6 : 20);
  ...
  $("#inspectorContent").innerHTML = `
    <section class="inspect-hero">
      ...
      <div class="inspect-actions"><button class="button primary" id="inspectorRun">▶ ${spec.isModel ? "Start / resume" : "Run selected"}</button><button class="button" id="inspectorSamples">⌾ Samples</button></div>
    </section>
    <section class="inspect-section">
      <div class="section-title">${spec.isModel ? "ML configuration" : "Configuration"}</div>
      ${spec.isModel ? `<div class="config-summary"><span><strong>${MODEL_CATALOG[spec.modelId].keys.length} live keys</strong><small>${MODEL_CATALOG[spec.modelId].modes.length} modes · ${escapeHtml(MODEL_CATALOG[spec.modelId].dataset)}</small></span><button class="button small primary" id="openFullConfig">Full config</button></div>` : ""}
      <div style="margin-top:${spec.isModel ? 9 : 0}px">${configEntries.map(([key, value]) => {
        ...
        return `<div class="form-row"><label>${escapeHtml(key.replaceAll("_", " "))}</label><input class="field inspector-config" data-key="${key}" value="${escapeHtml(value)}"></div>`;
      }).join("")}</div>
    </section>
    ...
    <section class="inspect-section"><div class="section-title">Latest artifact</div><article class="artifact-strip"><div class="artifact-strip-visual" id="artifactStrip">${previewGraphic(spec.visual, node.id.length + 3)}</div><footer><span><strong>${escapeHtml(spec.sampleLabel)}</strong><small>${escapeHtml(nodeVisualLabel(spec))}</small></span><button class="button small" id="artifactMini">Open</button></footer></article></section>
  ...
  $("#inspectorRun").addEventListener("click", () => {
    if (node.type === "optimize.design") openStudio("optimization");
  ...
  $("#inspectorSamples").addEventListener("click", () => openArtifact(node.id));
  $("#artifactStrip").addEventListener("click", () => openArtifact(node.id));
  $("#artifactMini").addEventListener("click", () => openArtifact(node.id));
