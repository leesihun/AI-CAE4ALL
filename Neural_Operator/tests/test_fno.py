...
def _build(h5_path, **overrides):
    cfg = base_config_2d(h5_path, model='fno', **overrides)
...
def _small_cfg(**extra):
    cfg = dict(fno_grid_resolution=[8, 8], fno_modes=[3, 4],
...
def test_ragged_batch_same_grid_shape(tiny_static_2d_h5):
    model, data_spec, domain, train, val, test, cfg = _build(tiny_static_2d_h5, **_small_cfg())
    ...
    for i, batch in enumerate(loader):
        if i >= 3:
            break
        grid_out = model.core.encode_operator(batch)
        grid_shapes.add(tuple(grid_out.shape[1:]))
    assert len(grid_shapes) == 1  # identical grid shape regardless of node count
...
def test_graph_isolation_no_cross_leakage(tiny_static_2d_h5):
    ...
    assert torch.allclose(pred_a_alone, pred_a_in_combined, atol=1e-3)
...
def test_overfit_tiny_fixture(tiny_static_2d_h5):
    ...
    for step in range(300):
        ...
        opt.step()
        last_loss = loss.item()
    assert last_loss < 0.1, f"failed to overfit tiny fixture, final loss={last_loss}"
