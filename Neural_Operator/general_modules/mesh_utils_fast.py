"""
Mesh visualization/writer utilities, adapted from MeshGraphNets'
general_modules/mesh_utils_fast.py (commit c559458).

Divergence from the MGN source (IMPLEMENTATION_PLAN.md section 5.3): this
repository's runtime graph contract has no `edge_attr` (no model here consumes
edge features), so the edge-attribute dataset is dropped from the HDF5 writer.
...
def plot_mesh_comparison(pos, faces, pred_values_norm, target_values_norm,
    ...
    cbar_label_denorm = ('Stress (Pa)' if is_stress
