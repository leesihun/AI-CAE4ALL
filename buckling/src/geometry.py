"""Analytic mapped 4-node shell meshes, generated in float64.

Deliberately NOT gmsh: gmsh writes text meshes at limited precision, and a
6-significant-digit coordinate file injects ~3e-4*t of symmetry-breaking
noise, which is above the physical imperfection signal (main.tex Sec 2.1's
dimples have to be resolved to well under that). Everything here is
closed-form in float64 and written at %.15e when serialized to a deck.

Element type is a 4-node quad shell (OpenRadioss QEPH, with Belytschko-Tsay
as a fallback property card if QEPH is too slow -- the connectivity below
is identical either way, only the property card differs). This replaces the
old buckling/ project's 8-node S8R mesh, which was CalculiX-specific and not
supported by OpenRadioss's standard shell library.

The node lattice is n_circ x (n_axial + 1), with the circumferential index
wrapping. No serendipity mid-side nodes (unlike the old S8R mesh).
"""
...
def mesh_counts(r_over_t: float, l_over_r: float, elems_per_half_wave: float = 12.0):
    ...
    Default is 12, not the old S8R mesh's 4: main.tex Sec 2.1 requires the
    mesh element size to stay <= sigma/10 (sigma = l_c, and l_c ~= lambda_half
    to within 0.3%), so the dimple-shape-preservation margin has to be
    enforced directly by element count here -- 4-node linear shells have no
    mid-side curvature correction to lean on the way S8R did.
    """
...
class ShellMesh:
    """Structured 4-node-quad mesh on a surface of revolution.
    ...
    def __init__(self, r_over_t, l_over_r, shape="cylinder", alpha_deg=0.0,
                 thickness_gamma=0.0, elems_per_half_wave=12.0, avoid_resonance=True):
    ...
    def thickness_at(self, z):
        """Axial thickness variation family (structural-OOD tier)."""
    ...
    def _build(self):
        NI = self.n_circ           # circumferential lattice, wraps
        NJ = self.n_axial + 1      # axial lattice

        idx = np.arange(NI * NJ, dtype=np.int64).reshape(NI, NJ)
        self.node_index = idx
        ...
        # ---- elements: 4-node quad, corners CCW starting bottom-left
        els = []
        for i in range(self.n_circ):
            ip1 = (i + 1) % NI
            for j in range(self.n_axial):
                els.append([idx[i, j], idx[ip1, j], idx[ip1, j + 1], idx[i, j + 1]])
        self.elements = np.asarray(els, dtype=np.int64)
        assert (self.elements >= 0).all(), "quad indexing produced a hole"
    ...
    @property
    def element_size_over_lc(self):
        """Circumferential element size / l_c -- must stay <= 0.1 (main.tex Sec 2.1)."""
        from imperfection import l_c
        h = 2.0 * np.pi * self.R / self.n_circ
        return h / l_c(self.R, self.t, NU)
    ...
    def summary(self):
        return dict(shape=self.shape, r_over_t=self.r_over_t, l_over_r=self.l_over_r,
                    ...
                    element_size_over_lc=self.element_size_over_lc)
