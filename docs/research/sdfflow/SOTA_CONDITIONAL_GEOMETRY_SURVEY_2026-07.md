...
2. **Hybrid VAE loss (TripoSG).** Replace L1-only with **SDF + surface-normal alignment + eikonal (`‖∇_x f‖ → 1`)**. Normal loss sharpens surfaces; eikonal makes the field a true metric SDF (better Marching Cubes, better downstream use by SDF-consuming operators). Needs grad-enabled query points (autograd through the decoder) — modest cost.
