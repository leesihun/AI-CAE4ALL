...
**날짜:** 2026-08-24
**대상:** `methods/SDFFlow/` (SDFFlow), 학습 완료된 `output/geometry_generation/ex1`
...
- **[A]가 기본.** 이미 `methods/GeometryIngest/readers.py::read_gmsh`가 gmsh를 쓰고 있고,
...
### 4.9 &nbsp;"조건을 만들 게 아니라 latent를 움직이면 되지 않나" — 측정해 본 결과

당연한 반문이다. 조건을 손으로 정의하는 대신, latent 축을 움직이면서 형상이 어떻게 변하는지
보면 되지 않나? **맞다. 그리고 이건 조건 설계의 반대편이 아니라 조건을 만드는 가장 싼 방법이다.**
ex1 체크포인트로 직접 재봤다.

**(a) 256차원 중 실제로 쓰는 건 27개다.**
전체 2138개 형상의 인코더 `mu`를 PCA한 결과 — 90% 분산에 22차원, **95%에 27차원**, 99%에 35차원.
탐색 가능한 규모다.

**(b) 단, raw dim이 아니라 PCA 축을 움직여야 한다.**
`kl_weight`가 1e-5라 축 정렬(disentanglement) 압력이 사실상 없다.
개별 `z[37]`을 움직이면 아무 의미 없는 방향으로 간다. PCA 축은 의미가 있다.

**(c) 현재 조건은 PC1 하나만 설명한다.** 상위 PCA 축과 descriptor의 상관:

| 축 | 분산 | 기존 4조건이 설명 | 확장 descriptor가 설명 |
|---|---|---|---|
| PC1 | 10.4% | volume −0.88, area −0.82, bbox_z +0.48 | convexity −0.84, sym_2 +0.93, inside_frac −0.76, planar_frac −0.64, max_inradius −0.58 |
| PC2 | 9.2% | **없음** | sym_3 −0.63 (비대칭 축) |
| PC3 | 9.0% | **없음** | pca_l2_l1 −0.59, sec_max −0.59, n_axis3 −0.58, sec_std −0.55 (세장비·단면 프로파일) |
| PC4 | 6.4% | **없음** | sec_min −0.42, pca_l3_l1 +0.44 (하중경로 병목) |
| PC5 | 6.0% | **없음** | max_inradius +0.42, sec_std +0.37 (벽두께) |
| PC6 | 5.7% | **없음** | n_axis3 −0.39 |
| PC7·PC8 | 5%+ | 없음 | **아직 이름 없음** |

**§1.3의 "조건 벡터가 비어 있다"의 시각적 증거다.** 모델은 이미 27개 축을 배웠는데
현재 조건은 그중 첫 번째 하나만 이름을 갖고 있다. §4.1의 확장 descriptor는
정확히 PC2~PC6에 이름을 붙이는 작업이다.

**(d) 거의 모든 descriptor가 latent에서 선형으로 읽힌다.** ridge, 80/20 split:

| descriptor | R² | | descriptor | R² |
|---|---|---|---|---|
| convexity | 0.978 | | n_axis1 | 0.870 |
| sym_2 | 0.972 | | max_inradius | 0.853 |
| pca_l3_l1 | 0.946 | | n_axis3 | 0.811 |
| pca_l2_l1 | 0.927 | | sec_std | 0.771 |
| sym_3 | 0.883 | | sec_max | 0.715 |
| volume (기존) | 0.976 | | planar_frac | 0.687 |
| area (기존) | 0.943 | | sec_min | 0.644 |
| | | | **sym_1** | **0.047** |

즉 **descriptor마다 latent 방향이 실제로 존재한다.** 재학습 없이 ridge 한 줄로 조종축을 만들 수 있다.
예외는 `sym_1`(0.047) — latent가 인코딩하지 않은 성질은 조종할 수 없고, 이건 §1.2의 인코더 문제로 돌아간다.

**(e) 한 방에 점프하면 부정확하지만, 측정→보정을 반복하면 압도적이다.**
`volume` 방향으로 목표만큼 이동 → 디코딩 → 실제 volume 측정 → 오차만큼 재이동. 중앙 상대오차:

| 목표 이동량 | 1회 | 2회 | 3회 | 4회 |
|---|---|---|---|---|
| +0.5σ | 5.4% | 1.4% | 0.4% | **0.1%** |
| +1.0σ | 7.3% | 1.4% | 0.2% | **0.0%** |
| +2.0σ | 8.6% | 1.0% | 0.1% | **0.0%** |
| −0.5σ | 7.6% | 3.2% | 1.4% | **0.6%** |
| −1.0σ | 18.1% | 8.9% | 4.7% | **2.5%** |
| −2.0σ | 98.7% | 60.5% | 35.9% | **9.9%** |

**비교 기준: 조건부 FM은 8.5%** (§1.4). 4회 반복이면 대부분 구간에서 **80배 이상 정확**하다.
디코딩 4회의 비용만 든다.

**비대칭에 주의.** +방향은 완벽하고 −방향은 나쁘다. 이유는 분포다 —
volume mean 0.2578, std 0.0763, **min 0.1198**. 즉 `mean − 2σ = 0.105`는 **데이터셋 밖**이고
`mean + 2σ = 0.410`은 max 0.4953 안쪽이다. **내삽은 되고 외삽은 안 된다**는 것이
숫자로 그대로 나온 것이다. 반복 보정이 경계를 어느 정도 밀긴 한다(98.7% → 9.9%).
±0.5σ 밖으로 나갈 때는 latent 공분산 기준 trust region이 필요하다.

#### 그래서 어느 쪽을 쓰나 — 역할이 다르다

| | latent 탐색·조종 | 조건부 학습 |
|---|---|---|
| **쓰는 때** | 뭐가 있는지 보기 · 기존 형상 편집 · 새 조건 나중에 추가 | 스펙으로 **새로** 대량 생성 |
| **비용** | 재학습 0. ridge 한 줄 | 데이터 재빌드 + 재학습 |
| **정확도** | 반복 시 0.1% (디코딩 4회) | 한 방에 8.5% |
| **안전 범위** | ±0.5σ 안전, ±2σ에서 붕괴 | 학습 분포 내 |
| **못 하는 것** | 다양성 (기존 형상 근처만) | 편집 (기존 형상 수정) |

**순서가 있다.**

1. **먼저 latent 탐색.** 오늘 재학습 없이 가능하고, "PC3이 뭔지" 알아내는 것이 곧
   "어떤 조건을 넣어야 하는지" 알아내는 것이다. **조건 설계를 추측이 아니라 데이터로 하게 된다.**
2. 거기서 얻은 이름을 **조건 벡터에 넣고 재학습.** 그래야 대량 생성과 다양성이 나온다.
3. 생성 후 **latent 반복 보정으로 마무리.** 8.5% → 0.1%.

셋 다 쓰되 역할이 다르다: 탐색은 *조건 설계 도구*, 조건 학습은 *대량 생성*, 반복 보정은 *마무리*.

> **Studio에 바로 넣을 수 있는 것:** 상위 PCA 축 10개를 슬라이더로 노출하고 실시간 프리뷰를 붙이면
> 재학습 없이 오늘 동작하는 대화형 형상 편집기가 된다. PC7 이후처럼 아직 이름이 없는 축도
> 그냥 슬라이더로 두면 된다 — 사용자가 움직여 보고 이름을 붙이는 게 descriptor 설계다.
