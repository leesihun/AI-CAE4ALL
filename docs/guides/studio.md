...
and output artifacts. The Studio itself remains under `studio/`; two narrow
...
- Mesh elements are reconstructed from the shared contract's `mesh_edge`
  ...
  See [studio_backend/mesh_topology.py](../../studio/studio_backend/mesh_topology.py).
...
job metadata, and logs are written only under the ignored `studio/runtime/`
