...
| `parallel_mode` | | `ddp`만 지원한다. GPU가 하나면 single-device worker를 사용한다. 과거 `parallelism/`의 `model_split` 코드는 VAE용 복사본이라 cHI flow 런타임에서 명시적으로 거부된다. |
...
| `std_noise` | | 레거시 설정의 `0`만 호환 목적으로 허용한다. 현재 cHI flow 학습기는 MGN 입력 노이즈를 구현하지 않으며 0이 아닌 값은 preflight와 네이티브 런처가 거부한다. |
