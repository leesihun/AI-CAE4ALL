...
class HIMGNSurrogate:
    ...
    def __init__(self, config_path, checkpoint, python=None, load_cases=('ver', 'dia'),
        ...
        # process, cwd=Geometry_generation) is in. A relative workdir resolved
    ...
    def analyze_batch(self, meshes, names=None):
        ...
        for sample_id, rec in enumerate(records, start=1):
            ...
            entry['cases'][rec['case']] = {
                ...
                # No solved system on this path to form force-times-displacement
                # from -- None (not 0.0) so it reads as "not measured" rather
                # than "measured and zero" wherever it surfaces in the report.
                'compliance': None,
            }
            results[index] = entry
...
class SurrogateEvaluator:
    ...
    forms). Those are set to inert placeholders rather than faked numbers.
    """
    ...
    def analyze(self, x, mc_resolution=None, target_faces=None, mesh_size_max=None,
        ...
        try:
            ...
            record['fea'] = {
                ...
                'num_nodes': result['num_nodes'], 'num_tets': 0,
                ...
                'max_compliance': 0.0,           # not formed on this path
            }
            record['mesh'] = {'num_nodes': result['num_nodes'], 'num_tets': 0,
                              'surrogate': True}
            record['ok'] = True
...
def _result_record(x, mesh, gen_info, result):
    ...
    record['mesh'] = {'num_nodes': result['num_nodes'], 'num_tets': 0, 'surrogate': True}
    return record
