...
/** Keep the legacy aligned columns while guaranteeing the native parser's
 * required whitespace separator for keys longer than the alignment width. */
export function flatConfigLine(key, value) {
  const name = String(key);
  return `${name}${" ".repeat(Math.max(1, 29 - name.length))}${value}`;
}
...
export function rawConfig(values, catalog) {
  ...
  const unknown = Object.keys(normalized)
    ...
    .sort();
  return [...ordered, ...unknown].map(key => flatConfigLine(key, normalized[key])).join("\n");
}
...
export async function applyPreset() {
  ...
  if (preset === "smoke" && spec.modelId === "simulgenvae") {
    ...
    if (!window.confirm("Create a tiny fixed-geometry HDF5 and condition CSV under studio/runtime, then load a one-epoch CPU SimulGen VAE config? This fixture exercises real code but is not scientific evidence.")) return;
    try {
