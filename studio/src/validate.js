...
export function geometryConfigText(node) {
  ...
  return `${Object.entries(values)
    ...
    .map(([key, value]) => flatConfigLine(key, value)).join("\n")}\n`;
}
...
function portRequiredInMode(node, port) {
  ...
  const pathFields = ["dataset_dir", "infer_dataset"].filter(key => required.has(key));
  if (!pathFields.length) return false;
  // Full config is a first-class input surface, not merely a reflection of
  // graph wiring. A user who deliberately selects a concrete dataset path
  // there can run a single model block; authoritative preflight still checks
  // that the path exists and matches the method contract. Without this, the
  // visible editable dataset_dir was ignored until an extra source block and
  // wire were added, and Train silently stopped at graph validation.
