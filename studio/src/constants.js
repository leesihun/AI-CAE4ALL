...
KEY_CATALOGS.sdfflow = [...new Set([
  ...
  "opt_seed",
  ...keys(`encoder_query_type posterior_min_std_rel split_by_parent vae_best_modelpath fm_best_modelpath latent_refine_steps latent_refine_lr latent_refine_prior_weight eval_split eval_num_shapes eval_seed interpolation_space`),
...
export const HELP = {
  ...
  fm_best_modelpath: "Where the best-validation FM checkpoint is written, alongside the last-epoch fm_modelpath. Same complete payload (the frozen VAE is embedded), so it is a stand-alone inference artifact. Safe to select on, unlike the VAE: the FM is the terminal stage, and its validation loss is the same quantity at every epoch because the FM objective has no warmup ramp (fm_warmup_epochs is learning-rate only).",
